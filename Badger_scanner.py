...
attribute_database = {}
output_file = os.getcwd() + '/BoldBadgers_attribute_floor.csv'
...
def checkItems():
    link = "https://83c6ddv6zu.medianet.work/nft_for_sale?collection=boldbadgers"
    headers = {
        ...
        for item in resp:
            ...
            if attribute_name in all_attributes:
                ...
                attribute_database[attribute_name] = sorted(
                    attribute_database[attribute_name])
            if attribute_name == "boldbadgers" and item.get('type') == attribute_name:
                attribute_database[attribute_name].append(item.get('price'))
                attribute_database[attribute_name] = sorted(
                    attribute_database[attribute_name])
    for attribute_name in attribute_set:
